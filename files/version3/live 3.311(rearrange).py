...
class MarketDashboard:
    ...
    def fetch_ltp(self):
        try:
            ...
            if response.get("data") and "ltp" in response["data"]:
                ...
                self.ltp_label.config(text=f"LTP:{ltp}")
                self.timestamp_label.config(text=f"Time: {timestamp}")
            else:
                self.ltp_label.config(text= f"LTP: Data not available")
        except Exception as e:
            self.ltp_label.config(text=f"Error fetching LTP: {e}")
        ...
        def update_ltp_label(self):
            ...
            self.root.after(10000,self.update_ltp_label)
    ...
    def fetch_data(self):
        ...
        intervals = [
            ...
            {"interval": "FIVE_MINUTE", "df_attr": "candlestick_data_5min", "hours": 5},
        ]
