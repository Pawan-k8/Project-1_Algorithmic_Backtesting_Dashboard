...
class MarketDashboard:
    ...
    def setup_ui(self):
        ...
        # Create LTP Display Frame (New)
        ltp_frame = ttk.LabelFrame(main_container, text="Live Trading Information")
        ltp_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Create LTP display with larger font and prominent styling
        self.ltp_label = ttk.Label(
            ltp_frame,
            text="LTP: --",
            font=("Helvetica", 16, "bold")
        )
        self.ltp_label.pack(side=tk.LEFT, padx=20, pady=10)
        ...
        self.timestamp_label.pack(side=tk.LEFT, padx=20, pady=10)
    ...
    def start_ltp_updates(self):
        """Start periodic LTP updates"""
        self.update_ltp_display()
        self.root.after(1000, self.start_ltp_updates)  # Update every second

    def update_ltp_display(self):
        """Update the LTP display labels"""
        try:
            ltp_data = self.fetch_ltp()
            if ltp_data:
                self.ltp_label.config(
                    text=f"LTP: ₹{ltp_data}",
                    foreground="green" if float(ltp_data) > 0 else "red"
                )
                self.timestamp_label.config(
                    text=f"Last Update: {datetime.now().strftime('%H:%M:%S')}"
                )
        except Exception as e:
            logger.error(f"Error updating LTP display: {e}")

    def fetch_ltp(self):
        """Fetch LTP from Smart API"""
        try:
    ...
    def fetch_data(self):
        ...
        try:
            # Fetch live market data first for quick LTP updates
