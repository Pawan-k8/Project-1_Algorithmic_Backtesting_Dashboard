...
class MarketDashboard:
    ...
    def is_market_open(self):
        ...
        market_end = datetime.strptime("16:30:00", "%H:%M:%S").time()
    ...
    def setup_ui(self):
        ...
        self.stocks_table = ttk.Treeview(table_frame, columns=("symbol", "ltp", "timestamp"), show="headings")
            ...
            for symbol in symbols_to_process:
                ...
                try:
                    ...
                    if response.get("data") and "ltp" in response["data"]:
                        ...
                        # Update in the UI
                        self.update_table_row(symbol, ltp, timestamp)
    ...
    def update_table_row(self, symbol, ltp, timestamp):
        # This needs to run in the main thread
        self.root.after(0, self._update_table_row, symbol, ltp, timestamp)
    
    def _update_table_row(self, symbol, ltp, timestamp):
        for item in self.stocks_table.get_children():
            values = self.stocks_table.item(item, 'values')
            if values and values[0] == symbol:
                self.stocks_table.item(item, values=(symbol, ltp, timestamp))
                break
