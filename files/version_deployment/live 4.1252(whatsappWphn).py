...
class StockScanner:
    def __init__(self):
        ...
        #whatsapp
        self.WHATSAPP_GROUP = wd.whatsapp_id  # Default group name
        self.PHONE_NUMBER = wd.phone_no
        self.whatsapp_enabled = tk.BooleanVar(value=False)
        self.whatsapp_patterns = set()
        self.whatsapp_pattern_var = tk.StringVar()
        self.whatsapp_patterns_label = None  # Initialize the label reference
        
        self.setup_ui()
        self.setup_whatsapp_config()
        self.generate_session()
    
    
    # Add these new methods to the StockScanner class:

    def setup_whatsapp_config(self):
        """Modified WhatsApp configuration setup"""
        # Create WhatsApp configuration frame
        whatsapp_frame = ttk.LabelFrame(self.root, text="WhatsApp Notifications")
        ...
        # Pattern selection for notifications
        pattern_combo = ttk.Combobox(
            whatsapp_frame,
            textvariable=self.whatsapp_pattern_var,
            values=self.available_patterns,
            width=20
        )
        ...
        ttk.Button(
            whatsapp_frame,
            text="Add Pattern",
            command=self.add_whatsapp_pattern
        ).pack(side=tk.LEFT, padx=5)
        ...
        # Create and store the patterns label
        self.whatsapp_patterns_label = ttk.Label(whatsapp_frame, text="Selected Patterns: None")
        self.whatsapp_patterns_label.pack(side=tk.LEFT, padx=5)


    def send_test_message(self):
        """Test WhatsApp messaging with better delay handling"""
        if not self.whatsapp_enabled.get():
            messagebox.showinfo("WhatsApp", "Please enable WhatsApp notifications first")
            return
        ...
        try:
            ...
            time.sleep(5)  # Ensure time buffer before showing messagebox
            messagebox.showinfo("WhatsApp", "Test message queued. Please check WhatsApp Web.")
        except Exception as e:
            logger.error(f"Error initiating test message: {e}")



    def _send_test_message_thread(self):
        """Thread function for test message"""
        try:
            message = (f"🔔 Test Alert!\n"
                      f"Scanner is running\n"
                      f"Group: {self.WHATSAPP_GROUP}\n"
                      f"Phone: {self.PHONE_NUMBER}\n"
                      f"Timestamp: {datetime.now().strftime('%H:%M:%S')}")
            self.send_whatsapp_message(message)
            logger.info("Test WhatsApp message sent successfully")
        except Exception as e:
            logger.error(f"Error in test message thread: {e}")

    def send_whatsapp_message(self, message):
        """Updated method to send WhatsApp messages correctly"""
        try:
            now = datetime.now()
            send_time = now + timedelta(minutes=2)  # Ensure time buffer
            
            if self.WHATSAPP_GROUP.startswith("G"):  # Group message
                pywhatkit.sendwhatmsg_to_group(
                    group_id=self.WHATSAPP_GROUP,
                    message=message,
                ...
                logger.info(f"WhatsApp message sent to group {self.WHATSAPP_GROUP}")
            
            else:  # Fallback to sending to individual phone number
                ...
                logger.info(f"WhatsApp message sent to number {self.PHONE_NUMBER}")
    
        except Exception as e:
            logger.error(f"Error sending WhatsApp message: {e}")
            messagebox.showerror("WhatsApp Error", f"Failed to send message: {str(e)}")


    def add_whatsapp_pattern(self):
        """Add pattern to WhatsApp notifications"""
        pattern = self.whatsapp_pattern_var.get()
        if pattern:
            self.whatsapp_patterns.add(pattern)
            self.update_whatsapp_patterns_label()

    def update_whatsapp_patterns_label(self):
        """Update the WhatsApp patterns label safely"""
        if hasattr(self, 'whatsapp_patterns_label') and self.whatsapp_patterns_label:
            patterns_text = ", ".join(self.whatsapp_patterns) if self.whatsapp_patterns else "None"
            self.whatsapp_patterns_label.config(text=f"Selected Patterns: {patterns_text}")
    
    def send_whatsapp_alert(self, symbol, pattern, price, trend):
        """Modified WhatsApp alert method"""
        if not self.whatsapp_enabled.get():
            return
        ...
        threading.Thread(
            target=self.send_whatsapp_message,
            args=(message,),
            daemon=True
        ).start()
    ...
    def setup_ui(self):
        ...
        pattern_frame = ttk.LabelFrame(main_container, text="Select Patterns to Scan",height=250)
    ...
    def clear_fields(self):
        ...
        # Clear WhatsApp patterns
        self.whatsapp_patterns.clear()
        self.update_whatsapp_patterns_label()
                        ...
                        try:
                            ...
                            if pattern_func(df):
                                ...
                                if ltp_data:
                                    ...
                                    # Send WhatsApp alert if pattern is in selected patterns
                                    if hasattr(self, 'whatsapp_patterns') and pattern in self.whatsapp_patterns:
