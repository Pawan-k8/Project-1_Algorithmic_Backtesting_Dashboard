...
class StockScanner:
    ...
    def setup_whatsapp_config(self):
        """Add this method to initialize WhatsApp configuration"""
        # Initialize WhatsApp settings
        self.whatsapp_enabled = tk.BooleanVar(value=False)
        self.whatsapp_number = tk.StringVar(value="")
        self.whatsapp_patterns = set()  # Store patterns to monitor
        ...
        # Phone number entry
        ttk.Label(whatsapp_frame, text="WhatsApp Number (with country code):").pack(side=tk.LEFT, padx=5)
        ttk.Entry(
            whatsapp_frame,
            textvariable=self.whatsapp_number,
            width=20
        ).pack(side=tk.LEFT, padx=5)
    ...
    def send_whatsapp_alert(self, symbol, pattern, price, trend):
        """Add this method to handle sending WhatsApp messages"""
        if not self.whatsapp_enabled.get() or not self.whatsapp_number.get():
            return
            
        try:
            phone_number = self.whatsapp_number.get()
            ...
            logger.info(f"WhatsApp alert sent for {symbol} - {pattern}")
