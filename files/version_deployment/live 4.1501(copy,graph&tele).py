...
class StockScanner:
    def __init__(self):
        ...
        self.setup_telegram()  # Add this line at the end of __init__
        
    
        # Add new utility methods after __init__:
    def setup_telegram(self):
        """Initialize Telegram bot with your credentials and send a test message."""
        try:
            self.bot = telegram.Bot(token=wd.bot_token)  # Replace with your bot token
            self.chat_id = wd.chat_id  # Replace with your group chat ID
            
            logger.info("Telegram bot initialized successfully")
            
            # Send a test message to verify connection
            asyncio.run(self.send_telegram_alert("🚦 Telegram bot connected successfully!"))
    ...
    async def send_telegram_alert(self, message):
        """Send message to Telegram with improved logging and error handling."""
        if self.bot and self.chat_id:
            try:
                logger.info("Attempting to send Telegram alert...")  # Log before sending
                await self.bot.send_message(
                    chat_id=self.chat_id,
                    text=message,
                    parse_mode='HTML'
                )
                logger.info(f"Telegram alert sent successfully! Message: {message}")
            except Exception as e:
                logger.error(f"Failed to send Telegram alert: {str(e)}")
        else:
            logger.warning("Telegram bot or chat ID not initialized.")
    



    def send_notification(self, symbol, pattern, price, trend):
        message = f"""🔔 <b>Pattern Detected!</b>
                    Symbol: {symbol}
                    Pattern: {pattern}
                    Price: {price}
                    Trend: {trend}"""
    ...
    def send_pattern_alert(self, symbol, pattern, data):
        """Format and send pattern detection alert immediately."""
        message = f"""🎯 <b>Pattern Detected!</b>
    ...
    Vol Ratio: {data.get('vol_ratio', 'N/A')}
    
    Time: {datetime.now().strftime('%H:%M:%S')}"""
        
        # Run the coroutine in a separate thread to avoid blocking
        threading.Thread(target=lambda: asyncio.run(self.send_telegram_alert(message))).start()


    
    def copy_to_clipboard(self, event):
        item = self.results_table.selection()[0]
        symbol = self.results_table.item(item)['values'][0]
        pyperclip.copy(symbol)
        self.show_tooltip(f"Copied {symbol} to clipboard!")
    ...
    # Add this new method to the StockScanner class
    def fetch_historical_data(self, symbol, token, exchange, minutes=60):
        """
        Fetch historical data for a specific duration
        """
        try:
            now = datetime.now()
            from_date = (now - timedelta(minutes=minutes)).strftime("%Y-%m-%d %H:%M")
            to_date = now.strftime("%Y-%m-%d %H:%M")
            
            response = self.smart_api.getCandleData({
                "exchange": exchange,
                "symboltoken": token,
                "interval": "ONE_MINUTE",  # Always use 1-minute candles
                "fromdate": from_date,
                "todate": to_date
            })
    ...
    def show_graph(self, event):
        ...
        graph_window.title(f"{symbol} - {pattern}")
        graph_window.geometry("800x600")
        
        # Fetch 60 minutes of 1-min data using the new method
        row = self.df_symbols[self.df_symbols['SymbolName'] == symbol].iloc[0]
        df = self.fetch_historical_data(symbol, str(row['token']), row['Exchange'], minutes=60)
        ...
        if not df.empty:
            ...
            # Customize the graph
            ax.set_title(f"{symbol} Last 60 Minutes")
            ax.set_xlabel("Time")
            ax.set_ylabel("Price")
            ax.grid(True, linestyle='--', alpha=0.7)
            
            # Format x-axis
            ax.set_xticks(range(0, len(df), 5))  # Show every 5th timestamp
            ax.set_xticklabels(df['datetime'].dt.strftime('%H:%M')[::5], rotation=45)
            
            # Add volume subplot
            ax2 = ax.twinx()
            ax2.bar(df.index, df['volume'], width, color='gray', alpha=0.3)
            ...
            # Add label with pattern information
            info_text = f"Pattern: {pattern}\nLast Price: {values[1]}\nVolume: {values[2]}"
            info_label = ttk.Label(graph_window, text=info_text, justify=tk.LEFT)
            info_label.pack(pady=5)
    
    def handle_table_click(self, event):
        region = self.results_table.identify('region', event.x, event.y)
        if region == "cell":
            item = self.results_table.identify('item', event.x, event.y)
            column = self.results_table.identify('column', event.x, event.y)
            
            # Column #0 is symbol, #4 is pattern
            if column == '#1':  # Symbol column
                self.copy_to_clipboard(event)
            elif column == '#5':  # Pattern column
    ...
    def setup_ui(self):
        ...
        # After creating the results_table and its scrollbar, add:
        self.results_table.bind('<ButtonRelease-1>', self.handle_table_click)
    ...
    # Now modify the scan_for_patterns method to integrate the alerts
    def scan_for_patterns(self):
        ...
        try:
                        ...
                        try:
                            if pattern_func(df):
                                ...
                                if ltp_data:
                                    ...
                                    latest_volume = df['volume'].iloc[-1] if not df.empty else 0  # Add this line
                                    ...
                                    alert_data = {
                                        ...
                                        'trend': latest_trend,
                                        'vol_ratio': f"{latest_vol_ratio:.2f}"
                                    }
                                    ...
                                    # Send Telegram alert
                                    self.send_pattern_alert(symbol, pattern, alert_data)
            ...
            if patterns_found > 0:
                summary_message = f"""
    📊 <b>Scan Complete</b>
    Found {patterns_found} pattern matches
    Scanned {total_symbols} symbols
    Time: {datetime.now().strftime('%H:%M:%S')}"""
                asyncio.run(self.send_telegram_alert(summary_message))
