...
SAFE_PERCENT = 0.04
TAX_RATE = 0.008
PROFIT_PERCENT = 0.065 #5.8 total profit from 5% profit and 0.8% tax
LOSS_PERCENT = 0.01    #1.8% total loss
...
class TradingDashboard:
    def __init__(self, root):
        ...
        self.create_rms_frame() #new RMS frame
    ...
    def create_rms_frame(self):
        frame = ttk.LabelFrame(self.root, text="RMS Data")
        frame.pack(fill="x", padx=10, pady=5)
        
        # Create Treeview for RMS data
        columns = ("Parameter", "Value")
        self.rms_tree = ttk.Treeview(frame, columns=columns, show="headings", height=2)
        self.rms_tree.pack(pady=5, fill="x")
        
        # Configure columns
        self.rms_tree.heading("Parameter", text="Parameter")
        self.rms_tree.heading("Value", text="Value")
        self.rms_tree.column("Parameter", width=400)
        self.rms_tree.column("Value", width=400)
        
        # Add refresh button
        refresh_btn = ttk.Button(frame, text="Refresh RMS", command=self.fetch_rms_data)
    ...
    def fetch_rms_data(self):
        try:
            # Clear existing data
            for row in self.rms_tree.get_children():
                self.rms_tree.delete(row)
    
            # Fetch and display new data
            rms_data = self.get_rms_data()
            if rms_data and rms_data.get("status") and "data" in rms_data:
                data = rms_data["data"]
                logger.info(f"RMS Data Response: {json.dumps(data, indent=4)}")
                # Display top 2 parameters
                top_2 = list(data.items())[:2]
                for key, value in top_2:
                    self.rms_tree.insert("", tk.END, values=(
                        key.capitalize().replace('_', ' '),
                        value
                    ))
    
                # Extract Net value and automatically trigger calculation
                net_value = data.get("net")
                if net_value is not None:
                    global XT
                    XT = float(net_value)  # Update global XT value
                    self.calculate()  # Trigger calculation
                else:
                    messagebox.showerror("Error", "Net value not found in RMS data")
            else:
                messagebox.showerror("Error", "Failed to fetch RMS data")
        except Exception as e:
            logger.error(f"Error updating RMS display: {e}")
            messagebox.showerror("Error", f"Failed to update RMS data: {e}")
    ...
    def calculate(self):
        try:
            ...
            messagebox.showerror("Error", "Please ensure valid RMS data is available")
            self.clear_fields() 
...
##this script will provide LTP, time, funds, token, multiplier, 